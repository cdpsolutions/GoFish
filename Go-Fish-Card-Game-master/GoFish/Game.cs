...
namespace GoFish
{
    class Game
    {
        ...
        public Game(string playerName, IEnumerable<string> opponentNames, TextBox textBoxOnForm)
        {
            ...
            players.Add(new Player(playerName, new Random(), textBoxOnForm));
            foreach(string s in opponentNames)
            {
                players.Add(new Player(s, new Random(), textBoxOnForm));
            }
            //add player to the list of players because before player list was empty
        ...
        public bool PlayOneRound(int selectedPlayerCard)
        {
            ...
            for(int i=0;i<players.Count;i++)
            {
                ...
                if(PullOutBooks(players[i]))
                {
                    ...
                    {
                        players[i].TakeCard(stock.Deal(i));
                        card++;
                    }
                }
